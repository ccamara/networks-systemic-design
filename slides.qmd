---
title: "Creating and visualising Networks with Gephi"
subtitle: "Networks: an option to visualise complexity"
...
# Preamble: **Food-Energy-Water nexus**
...
![](assets/figs/fwe/fwe_composite.png)
...
1.  Key concepts
2.  Learning by doing
    1.  Guided tasks: I will be demonstrating how to use a software and you will need to replicate the same steps
    2.  Bring Your Own Data (BYOD) Lab: here, you will be working autonomously with the dataset you've been producing.
...
::::: columns
::: {.column width="50%"}
A **network** is a *structure* in which two or more [entities]{.highlight} (vertices/nodes) are linked one to another by some kind of [relationship]{.highlight} via an edge (or link) that allow interaction, flow, or influence among the connected part.

Networks can be:

-   **Undirected**: these are reciprocal relationships where the direction of the link does not matter, $A → B = B → A$ (e.g. friendship, siblings)

-   **Directed**: relationshop where the source and target matters and are not reciprocal, $A → B \neq B → A$ (e.g. following/follower in social media, citations...)
:::
...
::::: columns
::: {.column width="50%"}
A **graph** is an abstraction of a network. I.e. it's how networks are stored, conceptualised and represented.
...
-   [**Nodes** (or vertices)]{.highlight}: usually identified with
    ...
    -   (optionally, but useful): a number of fields describing some features/attributes
...
-   [**Edges** (or links)]{.highlight}: defined as a pair of columns
    ...
    -   `unique ID`: a machine-readable unique identifier for each link (different from nodes)

    -   optionally, we may want to add labels, and attributes to edges.
:::

::: {.column width="50%"}
![First graph, made by Euler in 1736 to solve the problem of whether it was possible to visit Königsberg 4 quarters while crossing each of the 7 bridges only once](assets/figs/euler-graph.png)
:::
:::::
...
Not all networks are systems, but networks can be useful, for example, to identifying (parts of) entities working as systems.

| Dimension | System | Network | Graph |
|------------------|------------------|------------------|------------------|
| **Definition** | An organized collection of physically or conceptually **interrelated components** that [together achieve a **purpose** or exhibit emergent behavior]{.highlight}. | A set of entities (nodes) [linked by relationships]{.highlight}. (edges) that allow interaction, flow, or influence among them. | Mathematical model to store and conceptualise networks. |
...
-   [retina](https://ouestware.gitlab.io/retina/beta/): online, interactive data visualisation
-   `igraph`: library to create, analyse, visualise and export graphs from programming languages (`R` or `Python`) (out of scope)

# Guided task {background-color="#51D7C1"}

Using a provided network, we will be using Gephi to visualise and enquiry about our data.
...
The dataset contain 110 characters and 398 links from episodes I to VII.

-   two characters are connected if they speak together within the same scene.
-   data can be downloaded from [here (nodes)](https://github.com/ccamara/networks-systemic-design/raw/refs/heads/main/data/starwars_nodes.csv) and [here (edges)](https://github.com/ccamara/networks-systemic-design/raw/refs/heads/main/data/starwars_edges.csv)
...
We may want to use this data to get a better understanding of the dynamics between the characters in the Star Wars franchise, for example:
...
4.  Select Ranking
5.  Choose the **numeric attribute** (e.g. `Scenes`) you want to use to set the size
